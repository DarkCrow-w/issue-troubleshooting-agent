...
IDENTIFIER_PATTERNS = {
    "x_b3_spanid": re.compile(
        r"\bx-b3-spanid\s*[=:]\s*(?P<value>[\w.-]+)", re.IGNORECASE
    ),
    "x_b3_parentspanid": re.compile(
        r"\bx-b3-parentspanid\s*[=:]\s*(?P<value>[\w.-]+)", re.IGNORECASE
    ),
    "x_b3_traceid": re.compile(
        r"\bx-b3-traceid\s*[=:]\s*(?P<value>[\w.-]+)", re.IGNORECASE
    ),
    "x_request_id": re.compile(
        r"\bx-request-id\s*[=:]\s*(?P<value>[\w.-]+)", re.IGNORECASE
    ),
    ...
    "traceparent": re.compile(
        r"\btraceparent\s*[=:]\s*(?P<value>[\w-]+)", re.IGNORECASE
    ),
}
...
def _identifiers(raw: str, context: str) -> dict[str, str]:
    values = {
    ...
    for name, pattern in IDENTIFIER_PATTERNS.items():
        if match := pattern.search(raw):
            values.setdefault(name, match.group("value"))

    # comet gateway 会把 correlationId 写成 correlationId:[...]，不在标准上下文块中。
    bracketed = re.search(r"\bcorrelationId\s*:\s*\[(?P<value>[\w.:/-]+)]", raw)
    if bracketed:
        values.setdefault("correlationId", bracketed.group("value"))
    return values


def _payloads(message: str, kind: str) -> tuple[Any, Any]:
    request = None
    response = None
    if kind == "request":
        request = _json_after_marker(message, ("body is", "request is"))
    if kind == "response":
        response = _json_after_marker(message, ("body is", "response is"))
...
def parse_java_log(raw: Any) -> JavaLogDetails:
    ...
    request, response = _payloads(message, kind)
    return JavaLogDetails(
        ...
        request=request,
        response=response,
        identifiers=_identifiers(raw, context),
    )
